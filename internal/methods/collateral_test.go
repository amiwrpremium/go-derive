...
func TestGetCollaterals_Happy(t *testing.T) {
	...
	got, err := api.GetCollaterals(context.Background())
...
func TestGetCollaterals_Empty(t *testing.T) {
	api, ft := newAPI(t, true, 1)
...
func TestGetCollaterals_RequiresSubaccount(t *testing.T) {
	api, _ := newAPI(t, true, 0)
	_, err := api.GetCollaterals(context.Background())
